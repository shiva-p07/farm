...
exports.requestPasswordResetOTP = async (req, res, next) => {
  try {
    ...
    try {
      ...
      // Continue even if email fails - we'll return the OTP in development
    }

    // ALWAYS return OTP in the response for testing purposes
    // In production you would not include this
    console.log('Returning response with OTP:', otp);
    
    res.json({
      success: true,
      message: 'Password reset OTP has been sent to your email',
      otp // Include for testing, consider removing in production
    });
  } catch (error) {
