...
try {
  if (process.env.SENDGRID_API_KEY && process.env.SENDGRID_API_KEY.startsWith('SG.')) {
    ...
    console.log('Warning: SendGrid API key not found or invalid');
  }
} catch (error) {
 ...
 * @returns {Promise} - SendGrid response
 */
const sendPasswordResetOTP = async (user, otp, expiryMinutes = 10) => {
  try {
    // Load the template
    const template = loadTemplate('passwordResetOTP');
