...
const ForgotPasswordPage = () => {
  ...
  const handleSubmitEmail = async (e) => {
    ...
    try {
      ...
      // For development only - display the OTP that came from the server
      if (response.otp) {
        toast.info(`Development mode: Your OTP is ${response.otp}`);
        setDevModeOtp(response.otp);
        setOtp(response.otp); // Auto-fill the OTP field in development mode
      }
            ...
            <div>
              ...
              {devModeOtp && (
                <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
                  ...
                  <p className="text-sm text-blue-700">No emails are being sent in development mode.</p>
                  <div className="mt-2 flex items-center justify-center">
                    <p className="text-lg font-bold tracking-widest bg-white p-2 rounded border border-blue-300">
                      {devModeOtp}
                    </p>
                  </div>
                  <p className="mt-2 text-xs text-blue-600">This OTP has been auto-filled in the field below.</p>
                </div>
              )}
